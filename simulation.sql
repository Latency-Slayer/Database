...
BEGIN
    ...
    -- NOVO: Fator de redução geral de 50%
    DECLARE fator_reducao_geral FLOAT DEFAULT 0.5;
            ...
            WHILE servidor_atual <= 6 DO
                    ...
                    CASE servidor_atual
                        ...
                        WHEN 6 THEN
                            ...
                            IF dia BETWEEN 0 AND 4 THEN -- Segunda a Sexta
                                SET jogadores = FLOOR((200 + RAND() * 300) * crescimento * fator_tempo * multiplicador_jogo * reducao_por_alerta * fator_reducao_geral);
                            ELSE
                                SET jogadores = FLOOR((180 + RAND() * 250) * crescimento * fator_tempo * multiplicador_jogo * reducao_por_alerta * fator_reducao_geral);
                            END IF;
                        ...
                        WHEN 2 THEN
                            ...
                            SET jogadores = FLOOR((100 + RAND() * 180) * crescimento * fator_tempo * multiplicador_jogo * reducao_por_alerta * fator_reducao_geral);

                        -- Far Cry 5 (Servidor 1) - Terceira posição, estável
                        WHEN 1 THEN
                            SET fator_tempo = 1.0 + (dias_passados / 80.0) * 0.2;
                            SET multiplicador_jogo = 1.6;
                            SET jogadores = FLOOR((80 + RAND() * 140) * crescimento * fator_tempo * multiplicador_jogo * reducao_por_alerta * fator_reducao_geral);
                        ...
                        WHEN 4 THEN
                            ...
                            SET jogadores = FLOOR((90 + RAND() * 160) * crescimento * fator_tempo * multiplicador_jogo * reducao_por_alerta * fator_reducao_geral);
                        ...
                        WHEN 5 THEN
                            ...
                            SET jogadores = FLOOR((70 + RAND() * 120) * crescimento * fator_tempo * multiplicador_jogo * reducao_por_alerta * fator_reducao_geral);
                        ...
                        WHEN 3 THEN
                            ...
                            SET jogadores = FLOOR((40 + RAND() * 80) * crescimento * fator_tempo * multiplicador_jogo * reducao_por_alerta * fator_reducao_geral);
                        END CASE;
                    ...
                    IF hora BETWEEN 18 AND 23 THEN
                        CASE servidor_atual
                            ...
                            SET jogadores = jogadores + FLOOR(RAND() * 2800 * reducao_por_alerta * fator_reducao_geral);
                            WHEN 2 THEN -- Assassin's Creed: pico gradual
                            SET jogadores = jogadores + FLOOR(RAND() * 1800 * reducao_por_alerta * fator_reducao_geral);
                            WHEN 1 THEN -- Far Cry: pico moderado
                            SET jogadores = jogadores + FLOOR(RAND() * 1400 * reducao_por_alerta * fator_reducao_geral);
                            WHEN 4 THEN -- Watch Dogs: pico varia com fase do jogo
                            IF dias_passados > 90 THEN
                                SET jogadores = jogadores + FLOOR(RAND() * 600 * fator_tempo * reducao_por_alerta * fator_reducao_geral);
                            ELSE
                                SET jogadores = jogadores + FLOOR(RAND() * 2000 * fator_tempo * reducao_por_alerta * fator_reducao_geral);
                            END IF;
                            WHEN 5 THEN -- Star Wars: pico varia com crescimento
                            SET jogadores = jogadores + FLOOR(RAND() * 1200 * fator_tempo * reducao_por_alerta * fator_reducao_geral);
                            ELSE -- Skull and Bones: pico pequeno
                            SET jogadores = jogadores + FLOOR(RAND() * 800 * reducao_por_alerta * fator_reducao_geral);
                            END CASE;
                    END IF;
                    ...
                    IF dia IN (5, 6) THEN
                        CASE servidor_atual
                            ...
                            IF dia = 5 THEN -- Sábado
                                SET jogadores = jogadores + FLOOR(RAND() * 3500 * reducao_por_alerta * fator_reducao_geral);
                            ELSE -- Domingo
                                SET jogadores = jogadores + FLOOR(RAND() * 2200 * reducao_por_alerta * fator_reducao_geral);
                            END IF;
                            WHEN 2 THEN -- Assassin's Creed: pico gradual ambos os dias
                            SET jogadores = jogadores + FLOOR(RAND() * 1600 * reducao_por_alerta * fator_reducao_geral);
                            WHEN 1 THEN -- Far Cry: pico moderado
                            SET jogadores = jogadores + FLOOR(RAND() * 1200 * reducao_por_alerta * fator_reducao_geral);
                            WHEN 4 THEN -- Watch Dogs: varia com fase
                            IF dias_passados > 90 THEN
                                SET jogadores = jogadores + FLOOR(RAND() * 400 * fator_tempo * reducao_por_alerta * fator_reducao_geral);
                            ELSE
                                SET jogadores = jogadores + FLOOR(RAND() * 1800 * fator_tempo * reducao_por_alerta * fator_reducao_geral);
                            END IF;
                            WHEN 5 THEN -- Star Wars: pico gradual crescente
                            SET jogadores = jogadores + FLOOR(RAND() * 1000 * fator_tempo * reducao_por_alerta * fator_reducao_geral);
                            ELSE -- Skull and Bones: pico pequeno
                            SET jogadores = jogadores + FLOOR(RAND() * 600 * reducao_por_alerta * fator_reducao_geral);
                            END CASE;
                    END IF;

                    -- Garantir valor mínimo realista por jogo (reduzidos em 50%)
                    CASE servidor_atual
                        WHEN 6 THEN IF jogadores < 75 THEN SET jogadores = 75 + FLOOR(RAND() * 50); END IF;
                        WHEN 2 THEN IF jogadores < 40 THEN SET jogadores = 40 + FLOOR(RAND() * 35); END IF;
                        WHEN 1 THEN IF jogadores < 30 THEN SET jogadores = 30 + FLOOR(RAND() * 25); END IF;
                        WHEN 4 THEN
                            IF dias_passados > 90 THEN
                                IF jogadores < 5 THEN SET jogadores = 5 + FLOOR(RAND() * 10); END IF;
                            ELSE
                                IF jogadores < 20 THEN SET jogadores = 20 + FLOOR(RAND() * 30); END IF;
                            END IF;
                        WHEN 5 THEN
                            IF dias_passados <= 30 THEN
                                IF jogadores < 15 THEN SET jogadores = 15 + FLOOR(RAND() * 20); END IF;
                            ELSE
                                IF jogadores < 25 THEN SET jogadores = 25 + FLOOR(RAND() * 35); END IF;
                            END IF;
                        WHEN 3 THEN IF jogadores < 10 THEN SET jogadores = 10 + FLOOR(RAND() * 15); END IF;
                        END CASE;
                    ...
                    WHILE i < jogadores DO
                            ...
                            IF multiplicador_feriado > 1.0 AND reducao_por_alerta = 1.0 THEN
                                -- Chance de ser um jogador adicional por feriado (reduzida)
                                IF RAND() < ((multiplicador_feriado - 1.0) * fator_reducao_geral) THEN
